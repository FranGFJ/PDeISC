document.addEventListener('DOMContentLoaded', function() {
    ...
    document.getElementById('count-children').addEventListener('click', function() {
    const childCount = domTarget.children.length;
    document.getElementById('child-count').textContent = `Hijos: ${childCount}`;
    
    //resaltar los hijos momentáneamente
    Array.from(domTarget.children).forEach(child => {
        child.style.border = '2px solid red';
        setTimeout(() => child.style.border = '', 1000);
    });
    });
